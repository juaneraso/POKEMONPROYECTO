...
const getAddTypes = require("../controllers/getAddTypes");
...
const postPokemonHandler = async (req, res) => {
  try {
    const { name, image, hp, attack, defense, speed, height, weight, types } =
      req.body; //desestructuro datos del body
    const nameMinuscula = name.toLowerCase(); //convierto el nombre a minuscula

    const newPokemon = await postPokemones(
      nameMinuscula,
      image,
      hp,
      attack,
...
const getAddTypesHandler = async (req, res) => {
  try {
    const addTypes = await getAddTypes();
